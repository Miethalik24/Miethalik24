Hi, I'm Miethali Kulkarni 
---
🎯 Quality Engineer Analyst | Python Developer | Automation Enthusiast 
---
📍 Pune, India
***
🚀 About Me 

I’m a passionate Quality Engineer with hands-on experience in test automation, SAP testing, and Python scripting.
I enjoy designing efficient automation frameworks, simplifying workflows, and delivering high-quality software with precision and creativity.
Currently working at Accenture and exploring advanced Tosca, Selenium, and API testing techniques.
***
Automation & Testing 

🧩 Tosca · 💻 Selenium · 🧪 Manual & API Testing · 🧰 Test Design & Execution
***
Programming & Tools 

🐍 Python · 🧱 Django · ⚙️ REST APIs · 🗃️ SQL · 🧠 PyAutoGUI · 🧱 Excel Automation · 🗃️ SQL & PL/SQL · 🌐 HTML, CSS, JavaScript
***
Platforms & Environment

💼 SAP (ECC / S4HANA) · 🪟 Windows · ☁️ Citrix VDI · 🔄 Git & GitHub · 🐧 Linux (Ubuntu) · 🧩 Git & GitHub
***
Core Competencies:

Software Quality Assurance & Testing Strategy
Automation Framework Design (Tosca & Selenium)
API / Integration Testing & Validation
Defect Management & Root Cause Analysis
Test Data Management & Reporting
Continuous Improvement
Software Development Debugging & Optimization 
Object-Oriented Programming 
REST API Integration 
CI/CD Pipelines
***
💼 Major Projects

🩺 HealthCare Test Automation Suite Role: QA Automation Engineer Description: Built a comprehensive test automation suite for healthcare applications, automating 200+ regression and smoke test cases to enhance release quality. Designed reusable modules and integrated advanced reporting. Tech Stack: Tosca, Python, Postman, JIRA

⚙️ SmartAPI Testing Framework Role: API Automation Engineer Description: Created a scalable API testing framework supporting schema validation, automated response verification, and CI/CD integration. Improved test coverage by 40%. Tech Stack: Python, Postman, REST API, Jenkins

🌐 Playwright Automation Framework Role: Automation Framework Developer Description: Designed and implemented a Playwright-based cross-browser automation framework supporting UI, API, and integration testing with real-time HTML reports. Tech Stack: Playwright, Node.js, Jenkins, Allure Reports

💰 FinTrack – Personal Expense Manager Role: Backend Developer Description: Developed a full-stack web app to track and visualize personal spending habits with category-wise analytics. Tech Stack: Django, PostgreSQL, Chart.js

💻 LeetRank – Coding Progress Tracker Role: Full-Stack Developer Description: Created a web app that fetches and visualizes real-time LeetCode stats through REST APIs, enabling interactive performance tracking dashboards. Tech Stack: Flask, MongoDB, REST API

🧳 PortfolioHub – Developer Portfolio Website Role: Frontend Developer Description: Built and deployed a responsive portfolio website showcasing my projects, skills, and GitHub contributions dynamically. Tech Stack: React, Tailwind CSS
***
🏅 Certifications 

Tricentis Tosca Automation Specialist – Tricentis 
Playwright Advanced Automation – Udemy 
Data Analytics with Python – Great Learning 
GenAI for Test Automation – Accenture
***
🧠 Interests 

💡 Software Development | 🔐 Cyber Security | 🤖 Artificial Intelligence | 💻 Test Automation | 🧠 AI in QA | 🔍 API Testing | 🧩 Framework Design | 🌍 Web Development
***
📫 Connect With Me 

📧 Email: miethalikulkarni@gmail.com 
💼 LinkedIn: linkedin.com/in/miethali-kulkarni-31295b274/ 
💻 GitHub: github.com/Miethalik24 
🧠 LeetCode: leetcode.com/u/MiethaliKulkarni/
